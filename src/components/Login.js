...
import {isTokenExpired,refreshTokenExpired,encryptdata} from "../Helper/helper.js";
import { toast } from 'react-toastify';
const Login = (props) =>{
    const secretkey=process.env.REACT_APP_PUBLIC_KEY
   // console.log(secretkey,"fff");
    ...
    const handleFormSubmit= async(e)=>{
       ...
       // var CryptoJS = require("crypto-js");
        const datais=JSON.stringify(formData);
  
    const encryptvalue= await encryptdata(datais,secretkey);
    //console.log(encryptvalue,"New Value=======");

    var newencrypt_value={"data":encryptvalue};


        try {
           // const response = await axios.post("http://localhost:8081/api/login", formData);
            const response = await axios.post("http://localhost:8081/api/login", newencrypt_value);
